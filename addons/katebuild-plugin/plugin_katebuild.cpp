...
void KateBuildView::readSessionConfig(const KConfigGroup &cg)
{
    ...
    for (int i = 0; i < numTargets; i++) {
        ...
        for (int tn = 0; tn < targetNames.size(); ++tn) {
            ...
            m_targetsUi->targetsModel.addCommandAfter(cmdIndex, {.name = targetName, .buildCmd = buildCmd, .runCmd = runCmd, .env = {}});
        }
    }
...
QModelIndex KateBuildView::createCMakeTargetSet(QModelIndex setIndex, const QString &name, const QCMakeFileApi &cmakeFA, const QString &cmakeConfig)
{
    ...
    setIndex = m_targetsUi->targetsModel.addCommandAfter(setIndex,
                                                         {
                                                             ...
                                                             .buildCmd = QStringLiteral("%1 --build \"%2\" --config \"%3\" --parallel %4")
    ...
    setIndex = m_targetsUi->targetsModel.addCommandAfter(setIndex,
                                                         {
                                                             ...
                                                             .buildCmd = QStringLiteral("%1 --build \"%2\" --config \"%3\" --parallel %4 --target clean")
                                                                             .arg(cmakeFA.getCMakeExecutable())
                                                                             .arg(cmakeFA.getBuildDir())
                                                                             .arg(cmakeConfig)
                                                                             .arg(numCores),
    ...
    setIndex = m_targetsUi->targetsModel.addCommandAfter(setIndex,
                                                         {
                                                             .name = QStringLiteral("Rerun CMake"),
                                                             .buildCmd = QStringLiteral("%1 -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -B \"%2\" -S \"%3\"")
                                                                             .arg(cmakeFA.getCMakeExecutable())
                                                                             .arg(cmakeFA.getBuildDir())
    ...
    if (!cmakeGui.isEmpty()) {
        setIndex = m_targetsUi->targetsModel.addCommandAfter(
            setIndex,
            {
                .name = QStringLiteral("Run CMake-Gui"),
                .buildCmd = QStringLiteral("%1 -B \"%2\" -S \"%3\"").arg(cmakeGui).arg(cmakeFA.getBuildDir()).arg(cmakeFA.getSourceDir()),
            });
    }
    ...
    for (const QCMakeFileApi::Target &tgt : targets) {
        setIndex = m_targetsUi->targetsModel.addCommandAfter(setIndex,
                                                             {
                                                                 .name = tgt.name,
                                                                 .buildCmd = QStringLiteral("%1 --build \"%2\" --config \"%3\" --parallel %4 --target %5")
                                                                                 .arg(cmakeFA.getCMakeExecutable())
                                                                                 .arg(cmakeFA.getBuildDir())
                                                                                 .arg(cmakeConfig)
                                                                                 .arg(numCores)
...
void KateBuildView::slotAddTargetClicked()
{
    ...
    QModelIndex index = m_targetsUi->targetsModel.addCommandAfter(current, {.name = currName, .buildCmd = currCmd, .runCmd = currRun});
...
void KateBuildView::targetSetNew()
{
    ...
    QModelIndex buildIndex = m_targetsUi->targetsModel.addCommandAfter(index, {.name = i18n("Build"), .buildCmd = DefBuildCmd, .runCmd = QString()});
    m_targetsUi->targetsModel.addCommandAfter(index, {.name = i18n("Clean"), .buildCmd = DefCleanCmd, .runCmd = QString()});
    m_targetsUi->targetsModel.addCommandAfter(index, {.name = i18n("Config"), .buildCmd = DefConfigCmd, .runCmd = QString()});
    m_targetsUi->targetsModel.addCommandAfter(index, {.name = i18n("ConfigClean"), .buildCmd = DefConfClean, .runCmd = QString()});
...
void KateBuildView::addProjectTarget()
{
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            for (const auto &setVal : sets) {
                ...
                for (const auto &targetVals : obj[QStringLiteral("targets")].toArray()) {
                    ...
                    const QJsonObject buildEnvJson = tgtObj[QLatin1String("build_env")].toObject();
                    ...
                    m_targetsUi->targetsModel.addCommandAfter(setIdx, {.name = name, .buildCmd = buildCmd, .runCmd = runCmd, .env = buildEnv});
                    targetAdded = true;
                }
                if (targetAdded) {
    ...
    for (const QVariant &targetVariant : targetsets) {
        ...
        QPersistentModelIndex idx = m_targetsUi->targetsModel.addCommandAfter(set, {.name = tgtName, .buildCmd = buildCmd, .runCmd = runCmd, .env = buildEnv});
